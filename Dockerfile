...
# IMPORTANT: The base image already has CUDA paths set, but let's add compat path
# Check if compat directory exists and add it
RUN if [ -d "/usr/local/cuda/compat" ]; then \
        echo "Found CUDA compat directory, adding to library path"; \
        echo "/usr/local/cuda/compat" >> /etc/ld.so.conf.d/nvidia.conf; \
    else \
        echo "CUDA compat directory not found"; \
    fi && \
    ldconfig
